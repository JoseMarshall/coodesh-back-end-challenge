...
describe(makeDeleteOneEntity.name, () => {
  ...
  it('should delete a user from mongodb user collection', async () => {
    const user = colletions.users[0];

    const result = await makeSutRequest(sut({ model }), { [Common.MongoId]: user[Common.MongoId] });
    const validated = await deleteUserValidator(result);
    expect(validated).toBeDefined();
  });

  it('should get an error due to user not found', async () => {
    try {
      await makeSutRequest(sut({ model }), { [Common.MongoId]: 'nonexistent user id' });
    } catch (error) {
