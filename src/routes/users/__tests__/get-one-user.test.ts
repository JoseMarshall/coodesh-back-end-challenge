...
describe('Method GET /users/:userId should remove a user', () => {
  ...
  it('should return a 200 code response and a message', async () => {
    const response = await apiRequest
      .get(`/users/${user[Common.MongoId]}`)
      .set({ [Headers.APIKEY]: apikeys[2][ApiKey.Apikey], [Headers.Host]: apikeys[2][ApiKey.Host] })
      .send();
    const validated = await getOneUserValidator(response.body.payload);

    expect(response.status).toBe(200);
    expect(validated).toBeDefined();
    expect(response.body.msg).toContain(ApiMessages.FetchingEntitySuccess);
  });

  it('should return a 404 code response due to not found user', async () => {
    const response = await apiRequest
      .get(`/users/fd616d46-39b6-458d-a2c2-c306938cc320`)
      .set({
    ...
    expect(response.body.msg).toContain(ApiMessages.FetchingEntityError);
    expect(validated).toBeDefined();
  });
